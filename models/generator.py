...
import collections
import numpy as np
from utils.model_config import GENERATORCONFIGS, CNN_GENERATORCONFIGS, RESNET_GENERATORCONFIGS, RESNET20_GENERATORCONFIGS, VGG_GENERATORCONFIGS


class Generator(nn.Module):
    def __init__(self, dataset, model, embedding=False, latent_layer_idx=-1):
        super(Generator, self).__init__()
        self.embedding = embedding
        self.dataset = dataset
        #self.model=model
        self.latent_layer_idx = latent_layer_idx
        # noise_dim = GENERATORCONFIGS[dataset]
        print(model)
        if model == 'cnn':
            noise_dim = CNN_GENERATORCONFIGS[dataset]
        elif model == 'resnet18':
            noise_dim = RESNET_GENERATORCONFIGS[dataset]
        elif model == 'resnet20':
            noise_dim = RESNET20_GENERATORCONFIGS[dataset]
        elif model == 'vgg':
            noise_dim = VGG_GENERATORCONFIGS[dataset]
        else:
            noise_dim = GENERATORCONFIGS[dataset]
        self.hidden_dim, self.latent_dim, self.input_channel, self.n_class, self.noise_dim = noise_dim
